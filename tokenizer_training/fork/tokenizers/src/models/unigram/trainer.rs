...
pub struct UnigramTrainer {
    ...
    seed_size: usize,
    /// If provided, training starts from this vocabulary (with uniform scores)
    /// instead of running the suffix-array seeding step.
    #[builder(default = "None")]
...
impl UnigramTrainer {
    ...
    pub fn do_train(
        ...
        if let Some(seed) = &self.seed_vocab {
            // User-provided seed: use it as-is with uniform log-prob.
            // Score will be re-estimated by the first M-step.
            let n = seed.len() as f64;
            let uniform = -(n.ln());
            let mut seen: AHashSet<String> = AHashSet::new();
            for s in seed {
                if seen.insert(s.clone()) {
                    pieces.push((s.clone(), uniform));
                }
            }
        } else {
