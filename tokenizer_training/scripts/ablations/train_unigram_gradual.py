#!/usr/bin/env python3
"""
Train UnigramLM tokenizers with a small shrinking_factor for gradual pruning.
Same settings as train_compression_tokenizers.py — only shrinking_factor differs.

Names tokenizers as unigramlm_gradual_{sf}-{N}k where sf encodes the factor,
e.g. shrinking_factor=0.99 -> unigramlm_gradual_99-8k.
"""
...
# Dataset prep constants (match prepare_all.py)
...
DEFAULT_SHRINKING_FACTOR = 0.99   # gradual pruning: ~1% per iter
...
def train_unigram_gradual(vocab_size: int, corpus_iterator, shrinking_factor: float) -> Tokenizer:
...
def main():
    ...
    parser.add_argument("--shrinking-factor", type=float, default=DEFAULT_SHRINKING_FACTOR,
                        help="Fraction to keep per pruning iteration (e.g. 0.99 = drop 1%%)")
    args = parser.parse_args()

    sf = args.shrinking_factor
    # Encode as fractional digits: 0.9 -> "9", 0.99 -> "99", 0.999 -> "999"
    sf_tag = str(sf).split(".", 1)[1] if "." in str(sf) else str(sf)

    print(f"Training UnigramLM with shrinking_factor={sf}")
    print("=" * 60)
    ...
    for vocab_size in args.vocab_sizes:
        name = f"unigramlm_gradual_{sf_tag}-{vocab_size // 1000}k"
        print(f"\n{'='*50}")
        print(f"Training {name}")
        print(f"{'='*50}")

        tokenizer = train_unigram_gradual(
            vocab_size,
