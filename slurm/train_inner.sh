...
# Mirrors train_1B_inner.sh.
set -e

SEED=$1; TOK_NAME=$2; CONFIG_FILE=$3; PORT=$4
shift 4
RESUME_FLAG="$*"
...
# Pre-create dataset metadata single-process (shuffle_seed = training seed) so
# the DDP ranks load it instead of racing to create it.
...
torchrun --nproc_per_node="${CVL_NGPU}" --master_addr=localhost --master_port="${PORT}" \
