#!/usr/bin/env python3
"""
Absorbed-token analysis: how many merge results are just stepping stones?

Only applicable to BPE-based methods (BPE, BottomUpLL-Exact, BottomUpLL-Approx).
"""
...
def analyze_absorbed_tokens(merges, min_lengths=(0, 3, 5, 10)):
    merge_results = set()
    absorbed = set()
    ...
    survived = merge_results - absorbed

    results = {}
    for min_len in min_lengths:
        fc = {t for t in merge_results if len(t) >= min_len}
        fk = {t for t in absorbed if len(t) >= min_len}
        fs = {t for t in survived if len(t) >= min_len}
        results[min_len] = {"created": len(fc), "absorbed": len(fk), "survived": len(fs),
                            "ratio": len(fk)/len(fc)*100 if fc else 0}
    return results, absorbed, survived
...
def main():
    parser = argparse.ArgumentParser(description="Absorbed-token analysis (BPE methods only)")
    ...
    for name in names:
        ...
        results, absorbed, survived = analyze_absorbed_tokens(merges, args.min_lengths)
        all_absorbed[name] = absorbed
        all_survived[name] = survived

        print(f"\n{SHORT[name]}:")
        print(f"  {'Min len':>8}  {'Created':>10}  {'Absorbed':>10}  {'Survived':>10}  {'Kill %':>8}")
        print(f"  {'-'*8}  {'-'*10}  {'-'*10}  {'-'*10}  {'-'*8}")
        for ml in args.min_lengths:
            r = results[ml]
            print(f"  {ml:>8}  {r['created']:>10,}  {r['absorbed']:>10,}  {r['survived']:>10,}  {r['ratio']:>7.1f}%")
    ...
    if args.check_usage:
        ...
        for name in names:
            ...
            absorbed = all_absorbed[name]
            absorbed_ids = {vocab[t] for t in absorbed if t in vocab}
            zero = sum(1 for tid in absorbed_ids if usage[tid] == 0)
            low = sum(1 for tid in absorbed_ids if usage[tid] <= 10)
            any_use = sum(1 for tid in absorbed_ids if usage[tid] > 0)

            print(f"\n{SHORT[name]} ({len(absorbed_ids):,} absorbed tokens):")
            print(f"  Zero usage:  {zero:>7,} ({zero/len(absorbed_ids)*100:.1f}%)")
            print(f"  Low (≤10):   {low:>7,} ({low/len(absorbed_ids)*100:.1f}%)")
            print(f"  Has usage:   {any_use:>7,} ({any_use/len(absorbed_ids)*100:.1f}%)")

            absorbed_with_usage = [(id_to_token[tid], usage[tid])
                                 for tid in absorbed_ids if usage[tid] > 0 and tid in id_to_token]
            absorbed_with_usage.sort(key=lambda x: -x[1])
            print(f"  Top used 'absorbed' tokens:")
            for token, count in absorbed_with_usage[:10]:
