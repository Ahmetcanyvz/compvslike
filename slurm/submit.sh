...
read -r -a _toks  <<< "${TOKS:-bpe_count-128k compmax-128k greedyll-exact-128k unigramlm-128k}"
n=$(( ${#_seeds[@]} * ${#_toks[@]} ))
...
exec sbatch \
    ...
    --time="$CVL_SLURM_TIME" \
    --array=0-$((n - 1)) \
    --export=ALL,MODEL="${MODEL:-me1B-tied}",SEEDS="${SEEDS:-43 44}",TOKS="${TOKS:-bpe_count-128k compmax-128k greedyll-exact-128k unigramlm-128k}",BUDGET_B="${BUDGET_B:-20}" \
