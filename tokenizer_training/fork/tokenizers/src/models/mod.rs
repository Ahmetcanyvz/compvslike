...
use crate::models::unigram::{CompressionTrainer, Unigram, UnigramTrainer}; // + CompressionTrainer
...
pub enum TrainerWrapper {
    ...
    CompressionTrainer(CompressionTrainer), // NEW: greedy compression trainer
}
...
impl Trainer for TrainerWrapper {
    ...
    fn should_show_progress(&self) -> bool {
        match self {
            ...
            Self::CompressionTrainer(ct) => ct.should_show_progress(),
        }
    }
    ...
    fn train(&self, model: &mut ModelWrapper) -> Result<Vec<AddedToken>> {
        match self {
            ...
            Self::CompressionTrainer(t) => match model {
                ModelWrapper::Unigram(u) => t.train(u),
                _ => Err("CompressionTrainer can only train a Unigram".into()),
            },
        }
    }
        ...
        F: Fn(&str) -> Result<Vec<String>> + Sync,
    {
        match self {
            ...
            Self::CompressionTrainer(ct) => ct.feed(iterator, process),
        }
    }
}
...
impl_enum_from!(CompressionTrainer, TrainerWrapper, CompressionTrainer); // From<CompressionTrainer> for TrainerWrapper
