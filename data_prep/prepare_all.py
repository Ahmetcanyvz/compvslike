...
def tokenize_raw_data(
    ...
    if all((output_dir / s).exists() for s in ("train", "val", "test")):
    ...
    for split_name in ["train", "val", "test"]:
        ...
        # Count tokens in Arrow (materialising the column in Python would need
        # hundreds of GB at 20B tokens).
        try:
            import pyarrow.compute as pc

            total_tokens = pc.sum(
                pc.list_value_length(tok_ds.data.column("input_ids"))
            ).as_py()
