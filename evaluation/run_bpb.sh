#!/usr/bin/env bash
# Byte-per-byte evaluation for every trained model. One model per GPU,
# CVL_NGPU in parallel. Auto-discovers models; skips those already evaluated.
#
#   ./evaluation/run_bpb.sh                    # English test set
#   TEST_DATA=$CVL_RAW_MULTI/deu/test ./evaluation/run_bpb.sh
set -uo pipefail
source "$(dirname "$0")/../env.sh"

TEST_DATA="${TEST_DATA:-${CVL_RAW_EN}/test}"
...
for model in "${MODELS[@]}"; do
    ...
    echo "[GPU ${gpu}] bpb ${model} (tok ${tok_name})"
    CUDA_VISIBLE_DEVICES=$gpu python -m src.eval_bpb \
        "$ckpt" "$tokenizer" "$TEST_DATA" -o "$out" \
        > "${CVL_LOGS}/bpb_${model}.log" 2>&1 &

    ((launched++))
    (( launched % CVL_NGPU == 0 )) && wait
done
wait
echo "=== bpb done: launched=${launched} skipped=${skipped} -> ${CVL_EVAL_OUT} ==="
